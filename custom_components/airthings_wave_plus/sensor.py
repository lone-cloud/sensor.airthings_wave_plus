...
from homeassistant.const import (CONF_MAC,
                                 CONF_SCAN_INTERVAL, CONF_ELEVATION,
...
class BatterySensor(Sensor):
    ...
    def transform(self, value):
        ...
        battery_level = max(
            0, min(100, round((value-V_MIN)/(V_MAX-V_MIN)*100)))
...
DEVICE_SENSOR_SPECIFICS = {"date_time": Sensor('time', None, None, None),
                           ...
                           "radon_long_term_avg": RadonSensor(VOLUME_BECQUEREL, None, DEVICE_CLASS_RADON, 'mdi:radioactive')
                           }
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    if not hass.config.units.is_metric:
        DEVICE_SENSOR_SPECIFICS["radon_1day_avg"].set_unit_scale(
            VOLUME_PICOCURIE, BQ_TO_PCI_MULTIPLIER)
        DEVICE_SENSOR_SPECIFICS["radon_long_term_avg"].set_unit_scale(
            VOLUME_PICOCURIE, BQ_TO_PCI_MULTIPLIER)

    _LOGGER.debug("Searching for Airthings sensors...")
    airthings_detect = AirthingsWaveDetect(scan_interval, mac)

    try:
        if mac is None:
            num_devices_found = asyncio.run(
                airthings_detect.find_devices(scanner))
            _LOGGER.info(
                "Found {} airthings device(s)".format(num_devices_found))
        ...
        devices_info = asyncio.run(airthings_detect.get_info(hass))
        for mac, dev in devices_info.items():
            _LOGGER.info("{}: {}".format(mac, dev))

        _LOGGER.debug("Getting sensors")
        devices_sensors = asyncio.run(airthings_detect.get_sensors(hass))
        for mac, sensors in devices_sensors.items():
            for sensor in sensors:
                _LOGGER.debug("{}: Found sensor UUID: {} Handle: {}".format(
                    mac, sensor.uuid, sensor.handle))

        _LOGGER.debug("Get initial sensor data to populate HA entities")
        ha_entities = []
        sensor_data = asyncio.run(airthings_detect.get_sensor_data(hass))
        for mac, data in sensor_data.items():
            for name, val in data.items():
                _LOGGER.debug("{}: {}: {}".format(mac, name, val))
                ha_entities.append(AirthingsSensor(mac, name, airthings_detect,
                                   DEVICE_SENSOR_SPECIFICS[name]))
    except Exception as e:
        _LOGGER.exception("Failed initial setup: {}".format(e))
        return
...
class AirthingsSensor(SensorEntity):
    ...
    def extra_state_attributes(self):
        ...
        try:
            attributes[ATTR_DEVICE_DATE_TIME] = self.device.sensor_data[self._mac]['date_time']
        except KeyError:
    ...
    def update(self):
        ...
        value = self.device.sensor_data[self._mac][self._sensor_name]
